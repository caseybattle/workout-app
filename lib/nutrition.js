...
function toLb(weight, unit) {
  return num(weight) * (unit === "kg" ? LB_PER_KG : 1);
}
...
/* Multiplier is stored per lb-equivalent so lb and kg profiles share one model. */
export function targetKcal(profile) {
  return Math.round(toLb(profile?.weight, profile?.unit) * num(profile?.multiplier));
}
...
export function calibrate({ logs, weights, profile }) {
  ...
  const changeLb = toLb(last.avg - first.avg, profile.unit);
  const maintenance = avgIntake - (changeLb / span) * KCAL_PER_LB;
  const curLb = toLb(last.avg, profile.unit);
